...
Think about the goal zones as nodes (Tiles), one on each side of the board, that connnect to every Tile on that side of the board. Using an implementation like this you can think about checking whether someone has won as checking whether there is a path connecting two goal nodes where all the Tiles on that path (including the goal nodes) are of the same PlayerColor. In such an implementation you might want to give the goal zones coordinates using x and y values of -1 or size (where size is the size of any side of the board). Choosing an implementation like this might simplify the writing of your conditional checks when trying to determine whether a player has won or what the neighbors of a Tile are. [Board.jpg](./Board.jpg) in this folder shows an ASCII example of this sort of board representation.

SimpleGameBoard inherits from a class as well as an interface. This means that it must implement the three methods from Board that do not already have a body in Board, namely getNeighbors, makeMove and getTileAt. SimpleGameBoard also has two constructors, similar to Tile, one that explicitly gives values for the class fields, and another that takes an existing Board and makes a copy. See the [javaDocs](https://github.iu.edu/pages/c212-fall2014/project/apidocs/overview-summary.html) for more information on those constructors.
...
Previous homework and lab assignments are a useful resource, please refer to them if you have trouble remembering how to do something as they contain examples of everything you will need to do for this assignment.  Almost all of your homework assignments and many of your labs will include examples that do/use the following things, so you should consider the list below to be a selection of references and not a comprehensive list of references.  

Overriding Equals: Lab6, A7

Overriding toString: Lab6, A7

Inheritance: Lab6, A7

Abstract Classes: Lab6, A7
...
Nested for loops: A2, A4

Getters and Setters: Every assignment

Point: A5

###Hashing (This section is optional)

The following (on [hashCodes](http://docs.oracle.com/javase/7/docs/api/java/lang/Object.html#hashCode%28%29)) is in no way required in order to successfully complete this milestone or the project in general, but some may find it interesting and the material below is generally useful and relevant to programming and computer science more broadly:

In this assignment you will need to work with Sets, and we will encourage you to use a HashSet. HashSets use something called a hash (generated using a hashing function or an Object's hashCode method in Java).  This hash allows elements to be found very quickly in a hashed data structure, frequently yielding significant performance increases when searching for an item in a data structure (e.g. a Set), as you can go directly to that item's location in the data structure instead of searching the data structure for it one item at a time until you find the item you're looking for.  Without going too deeply into the details of hashing, one of the goals of a hashing function (hashCode method in Java) is to generate a unique integer output using the fields of the item it is hashing.  For example, in your Move class, you would want to use the x and y coordinates representing the location of an instance of Move on the board in order to generate a unique hash for that instance.  While writing a hashing function for something like the Move class is not a simple mathematical problem, the solution to an equivalent problem has already been implemented in the hashCode method of Java's Point class.  As a result, when dealing with a HashSet of moves, you will be able to increase the performance of your code somewhat (and maybe learn a little bit about hashing and hashed data structures too) by overriding the hashCode method that Move inherits from Object and instead calling the hashCode method of the Move's point.

As an additional note: I would encourage you to google hashing and hashed data structures like [HashMap](http://docs.oracle.com/javase/7/docs/api/java/util/HashMap.html) and [HashSet](http://docs.oracle.com/javase/7/docs/api/java/util/HashSet.html) further.  You will find hashed data structures, especially [HashMaps](http://docs.oracle.com/javase/7/docs/api/java/util/HashMap.html), to be very useful when coding in general and you will be expected to use them and know them when interviewing for many software engineering internships and jobs.  Having said that, this is not a data structures or algorithms class and you do not need to know anything about hashing or hashed data structures for this course.  To say that a different way, the material above is not required for the project and will not be on any exams, so don't be worried if the above doesn't make any sense to you right now.  You do not need to override the hashCode method in any of your classes and HashSets will still work in the classes you write because they will use the default hashCode implementation that they inherit from Object.
